...
def crear_barra_menu():
    ...
    # Crear menú de Créditos
    menu_creditos = tk.Menu(barra_menu, tearoff=0)
    barra_menu.add_cascade(label="Créditos", menu=menu_creditos)
    menu_creditos.add_command(label="Ver Créditos", command=mostrar_creditos)
...
root = tk.Tk()
root.title("Generador de Curva Floral")
...
ttk.Label(root, text="Nombre Científico:").grid(row=1, column=0, padx=10, pady=10)
nombre_cientifico_entry = ttk.Entry(root)
nombre_cientifico_entry.grid(row=1, column=1, padx=10, pady=10)
